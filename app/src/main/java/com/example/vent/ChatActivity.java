...
public class ChatActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // getting the other user's id passed from ChatsFragment
        ...
        // dialog box will pop-up when user wishes to send a file
        // for now user can only send images
            ...
            public void onClick(View v) {
                ...
                builder.setItems(options, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        //if users wishes to send image, sent to the photo library
                        if(which == 0){
                            checker = "image";
                            Intent intent = new Intent();
                            intent.setAction(Intent.ACTION_GET_CONTENT);
                            intent.setType("image/*");
                            // start photo library with result so that we can retreive the image the user wishes to send
    ...
    // saves the file in both the users 'messages' node --> "Database/Messages/UserId/"
    // recycler view refreshes everytime a new message is saved (onChildAdded)
    // which is what appears as sending the message
    @Override
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if(requestCode==438 && resultCode == RESULT_OK && data!=null && data.getData() != null){
            ...
            if(!checker.equals("image")){
                // for retrieving and sending documents
                ...
                // retrieves image from the mobile
                File compressedFilePathUri = new File(fileUri.getPath());

                //compress the image to 30% quality to reduce storage space
                ...
                uploadTask.continueWithTask(new Continuation() {
                    ...
                    public void onComplete(@NonNull Task<Uri> task) {
                        ...
                        // when image is sucessfuly uploaded to FirebaseStorage
                        // image is saved in database as message(for both the users) using url of image saved in FirebaseStorage
                        if(task.isSuccessful()){
    ...
    // displays the last time user was using the app
    ...
    protected void onStart() {
        ...
        //displays all the messages sent b/w the current user and the other user for the current user
        // i.e. the messageSender(current user) and messageReciever(other user)
    ...
    // clear the messages list when the page is closed
    @Override
    ...
    // saves text message in each user (sender and receiver) 'messages' node --> "database/Messages/UserID/'
    // recycler view refreshes everytime a new message is saved (onChildAdded)
    // which appears as 'sending' the message to the other user
